...
> **Update (2026-08-08, deck Path A / template support):** §U grew 4 → **6 cases** — `Deck.templateId` (nullable FK to `BrandKitTemplate`) is now threaded through `POST /api/decks` (eagerly validated: 400 on a template from the wrong kit or the wrong aspect ratio, 404 on a deleted/nonexistent id — never a 500 at outline-approve, which was the gap this closed) and `generateDeck.ts`'s per-slide `createPendingDraft` call, so a Path A deck's slides now generate via the template-fill pipeline instead of hard-defaulting to Path B `GENERATE` mode. Two new cases: a Path A deck's outline-approve creating slides that reach `EXPORTED` via `assemble-a`-equivalent template fill, and a bad Path A template selection (wrong kit/ratio) rejected with a 4xx at creation. `GET /api/decks/[id]` returns `templateId`; the wizard (`useDeckWizard.ts`/`DeckSizeStep.tsx`) gained the Path A/B toggle + `TemplateCard` picker (ported from the single-brief wizard), filtered to the selected kit + aspect ratio, with dangling-id clearing on kit/ratio change. Unit: `tests/unit/deck/generateDeck.test.ts` gained cases asserting `createOneSlide` passes `deck.templateId` through (both set and null). **Confirmed green 2026-08-08** — full mock E2E catalog run, `deck-generation.test.ts` 6/6 pass; the only other failures in that run were the pre-existing browser-login/HMR `page.waitForURL` timeout flakes (`ui.test.ts`, `campaign-scheduling.test.ts`, `settings-claude-token.test.ts`) plus one cascading `team-isolation.test.ts` fixture-setup failure from the same root cause — none touch this change's diff.
>
