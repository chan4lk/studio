...
- [x] `T1` — `Deck.templateId` column + migration
...
- [x] `T2` — `POST /api/decks`: accept + eagerly validate `templateId`
...
- [x] `T3` — Wire `templateId` through `generateDeck.ts` + fix approve-route error mapping
...
- [x] `T4` — `GET /api/decks/[id]`: return `templateId`
...
- [x] `T5` — `useDeckWizard.ts`: template state, query, filtering, dangling-id clear
...
- [x] `T6` — `DeckSizeStep.tsx`: Path A/B toggle + template picker
